...
*/
angular.module('authoringEnvironmentApp').service('circularBufferFactory', function circularBufferFactory() {
    /**
    ...
    this.create = function (opt) {
        if (!('size' in opt)) {
            throw new Error('a circular buffer needs an integer size option');
        }
        return {
            opt: opt,
            arr: [],
            ...
            prev: function (optionalCount) {
                ...
                if (intCount < 1) {
                    throw new Error(intCount + ' is not a valid value for the `prev` function, check the tests or the code');
                }
