			...
			.run(['$httpBackend', function($httpBackend) {
			    ...
			    var snippets = {
				    ...
				    8: snippetGenerator(8, 'Tweet')
                            ...
                            var re = new RegExp('/content-api/images[?]page=[0-9]+');
                            ...
                            var re = new RegExp('/content-api/images/[0-9]+');
                            $httpBackend.whenGET(re).respond({
                                "id":1,
                                "location":"local",
                                "basename":"French_election__N_2200069b.jpg",
			    ...
			    $httpBackend.whenGET(/\/content-api\/article\/64\/1\/snippets\/[\d]/).respond(function(method, url, data) {
			    	console.log(url.split('/'));
			    	return [200, snippetGenerator(url.split('/')[4], 'generic')];
			    });

			    $httpBackend.whenGET(/\/content-api\/article\/64\/1\/snippets/).respond(snippetsForArticle);
			    
			    $httpBackend.whenGET(/\/content-api\/snippets\/generic\/[\d]/).respond(function(method, url, data) {
			    	return [200, snippetGenerator(url.split('/')[4], 'generic')];
			    });
			    $httpBackend.whenGET(/\/content-api\/snippets\/tweet\/[\d]/).respond(function(method, url, data) {
			    	return [200, snippetGenerator(url.split('/')[4], 'tweet')];
			    });
			    $httpBackend.whenGET(/\/content-api\/snippets\/generic/).respond(snippetsGeneric);
			    $httpBackend.whenGET(/\/content-api\/snippets\/tweet/).respond(snippetsTweet);
			    $httpBackend.whenGET(/\/content-api\/snippets/).respond(snippets);
			    $httpBackend.whenGET(/\/content-api\/snippetTemplate\/[\d]/).respond(snippetTemplate);

			    // $httpBackend.whenGET(/content-api\/.*/).passThrough();
